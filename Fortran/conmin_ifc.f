	...
	SUBROUTINE CMINEX (X, VLB, VUB, ISC, NDV, NCON, NSIDE,
     &             IPRINT, ITMAX, IER, N1, N2, N_INT, ICHROM_INT, 
     &             FITNESS, ASTEROID)
...
!**                                                                   **
!**  NOTES:                                                           **
C**    1. Argument variable type declarations:                        **
C**       a) REAL:     X, VLB, VUB                                    **
C**       b) INTEGER:  ISC, NDV, NCON, NSIDE, IPRINT, ITMAX, IER      **
C**    2. Single precision arithmetic is used throught the entire     **
C**       subroutine.                                                 **
C**    3. The program module (main program, subroutine, etc.) which   **
C**       calls CMINEX should contain an EXTERNAL statement for the   **
C**       name of the subroutine which will evaluate the objective    **
C**       function, OBJ, and the constraints, G(I), I=1,NCON.         **
C**    4. All of the output from CMINEX and CONMIN will be sent to    **
C**       the screen.  If you want to have this output in a file, try **
C**       piping the output to a file, i.e., if your executable file  **
C**       is called example.x use the following UNIX command:         **
C**       UNIX:  example.x >example.out                               **
C**       This will cause everything that was sent to the screen to   **
C**       be sent to the file example.out instead. Thus, example.x    **
C**       could not be an interactive program since all of the screen **
C**       prompts would end up in example.out and not on the screen.  **
C**    5. The named COMMON blocks CNMN1 and CONSAV are used by CMINEX **
C**       and the CONMIN subroutines.  Thus the user should not have  **
C**       any named COMMON blocks with the names CNMN1 or CONSAV.     **
C**    6. The CONMIN subroutines are named CNMN01, CNMN02, CNMN03,    **
C**       CNMN04, CNMN05, CNMN06, CNMN07, and CNMN08.  Thus, the user **
C**       should not have any subroutine, function, or block data     **
C**       subprograms with these names.                               **
C**    7. The dimensions in this subroutine are currently set for a   **
C**       maximum of 20 independent (design) variables and a maximum  **
C**       of 10 constraints.  In the program module which calls       **
C**       CMINEX, the arrays X, VLB, VUB, and ISC should have the     **
C**       following dimensions:                                       **
C**       X(MAXN1), VLB(MAXN1), VUB(MAXN1) where MAXN1 = 22.          **
C**       ISC(MAXN2)                       where MAXN2 = 50.          **
C**    8. Of the argument variables, only the array X will have its   **
C**       values modified (AS EXPLAINED ABOVE) on exit from CMINEX.   **
C**                                                                   **
C**  REFERENCE:                                                       **
C**    "CONMIN - A FORTRAN Program for Constrained Function           **
C**     Minimization, User's Manual", G. N. Vanderplaats,             **
C**     NASA TMX-62,282, Aug., 1973.                                  **
C**                                                                   **
...
C**  User Supplied:                                                   **
C**  If NCON >= 0:                                                     **
C**    ANALYZ (X, NDV, OBJ, G, NCON)                                   **
C**      A subroutine which will evaluate the objective function, OBJ **
C**      and the constraints, G(I), I=1,NCON, at the given set of     **
C**      independent (design) variables, X(J), J=1,NDV.               **
C**      REAL:      X, OBJ, G                                         **
C**      INTEGER:   NDV, NCON                                         **
C**                                                                   **
C***********************************************************************
...
C
     ...
     &               ICNDIR, IGOTO, NAC, INFO, INFOG, ITER
      DIMENSION X(MAXN1), VLB(MAXN1), VUB(MAXN1), G(MAXN2), SCAL(MAXN1),
     &          DF(MAXN1), A(MAXN1,MAXN3), S(MAXN1), G1(MAXN2),
     &          G2(MAXN2), B(MAXN3,MAXN3), C(MAXN4), ISC(MAXN2),
     &          IC(MAXN3), MS1(MAXN5), ASTEROID(N1,N2), 
     &          ICHROM_INT(N_INT)


C
C-----Making the argument list and the COMMON block compatible:
C
      ...
      IDUM5 = ITMAX
C
C-----Check to be sure that the problem is not too big
C-----for the current set of parameters:
C
...
C
      ...
      ABOBJ1 = 0.0
      THETA  = 0.0
C	ASH: IF NFDG = 0	THEN GRADIENT IS CALCULATED BY FINITE DIFF METHOD
C	ASH: IF NSCAL =0	THEN INDEPENDANT VARIBALES ARE NOT SCALED 
      NFDG   = 0
      NSCAL  = 0
      LINOBJ = 0
      ITRM   = 5
...
C
      ...
      END IF
C
C-----Evaluate the objective function and the constraints:
C
      CALL COST(NDV, N_INT, N1, N2, X(1:NDV), ICHROM_INT, OBJ,   
     &          ASTEROID, G(1:NDV), NCON)
         FITNESS=OBJ

      GO TO 10
C
C-----Exit from the subroutine:
C
c      RETURN
      END
     ...
     *RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
      DIMENSION X(N1),VLB(N1),VUB(N1),G(N2),SCAL(N1),DF(N1),A(N1,N3),S(N
     *1),G1(N2),G2(N2),B(N3,N3),C(N4),ISC(N2),IC(N3),MS1(N5)
     ...
     *ALP,FFF,A1,A2,A3,A4,F1,F2,F3,F4,CV1,CV2,CV3,CV4,APP,ALPCA,ALPFES,A
     *LPLN,ALPMIN,ALPNC,ALPSAV,ALPSID,ALPTOT,RSPACE,IDM1,IDM2,IDM3,JDIR,
...
c**********************************************************************c
c*****                                                            *****c
c*****               This version of CONMIN was                   *****c
c*****               modified to execute on the                   *****c
c*****               APOLLO SERIES 3000.  The                     *****c
c*****               source code was compiled                     *****c
c*****               using the FORTRAN 77                         *****c
c*****               compiler.                                    *****c
c*****                                                            *****c
c*****               Jerald M. Vogel                              *****c
c*****               Iowa State University                        *****c
c*****               September 26, 1983                           *****c
c*****                                                            *****c
c*****                                                            *****c
c*****                                                            *****c
c**********************************************************************c
...
C!HINDMAN 3/4/2002      IF (DELFUN.LE.0.) DELFUN=.0001
      IF (DELFUN.LE.0.) DELFUN=.00000000001
      CT=-ABS(CT)
      IF (CT.GE.0.) CT=-.1
      CTMIN=ABS(CTMIN)
C	ASH: CTMIN ORGINAL = 0.004
      IF (CTMIN.LE.0.) CTMIN=.004
      CTL=-ABS(CTL)
      IF (CTL.GE.0.) CTL=-0.01
      CTLMIN=ABS(CTLMIN)
      IF (CTLMIN.LE.0.) CTLMIN=.001
C	ASH: 	THIS DROPPED THE OBJ WHEN THETA = 1 TO 10
C		ALSO, THE RESULTS BETWEEN THE LINEAR AND COSINE DISTR MATCHED			
C		WHEN ALL THESE VALUES WERE DROPPED THE ITER WAS MUCH LESS
C		AND THE OBJ WENT DOWN TO 10-6 MAGNITUDE 
C      IF (THETA.LE.0.) THETA=1000.0
C      IF (ABOBJ1.LE.0.) ABOBJ1=.000001
C      IF (ALPHAX.LE.0.) ALPHAX=.000001
C      IF (FDCH.LE.0.) FDCH=.000001
C      IF (FDCHM.LE.0.) FDCHM=.000001
...
150   CONTINUE
      OBJ1=OBJ
C!HINDMAN 3/4/2002      IF (DABFUN.LE.0.) DABFUN=.001*ABS(OBJ)
...
C     ------------------------------------------------------------------
      ...
      WRITE (6,1250) IPRINT,NDV,ITMAX,NCON,NSIDE,ICNDIR,NSCAL,NFDG,LINOB
...
270   CONTINUE
      ITER=ITER+1
C	ASH IF THIS IS DROPPED THAT ITER GOES WAY UP !!!
...
C	ASH
      NFEAS=NFEAS+1
c	ASH 10/22/02      IF (NFEAS.GT.10) GO TO 790
...
C     ------------------------------------------------------------------
      JGOTO=0
650   CONTINUE
      CALL CNMN03 (X,S,SLOPE,XI,ALP,FFF,A1,A2,A3,A4,F1,F2,F3,F4,APP,N1,N
     *CAL,KOUNT,JGOTO)
      IGOTO=4
      IF (JGOTO.GT.0) GO TO 950
      JDIR=1
...
660   CONTINUE
      JGOTO=0
670   CONTINUE
      CALL CNMN06 (X,VLB,VUB,G,SCAL,DF,S,G1,G2,CTAM,CTBM,SLOPE,XI,ALP,A2
     *,A3,A4,F1,F2,F3,CV1,CV2,CV3,CV4,ALPCA,ALPFES,ALPLN,ALPMIN,ALPNC,AL
     *PSAV,ALPSID,ALPTOT,ISC,N1,N2,NCAL,NVC,ICOUNT,IGOOD1,IGOOD2,IGOOD3,
...
C     ABSOLUTE CHANGE IN OBJECTIVE.
      ...
      OBJB=ABS(OBJD)
C	ASH
C	10/21/02 IF (OBJB.LT.1.0E-10) OBJB=0.
...
C     ------------------------------------------------------------------
C	ASH
C	10/21/02 IF (ABS(OBJ1).GT.1.0E-10) OBJD=OBJD/ABS(OBJ1)
...
C     ------------------------------------------------------------------
C     STOP IF OBJ IS LESS THAN -1.0E+38.
...
980   FORMAT (///5X,56HCONMIN HAS ACHIEVED A SOLUTION OF OBJ LESS THAN - 	&
     11.0E+38/5X,32HSOLUTION APPEARS TOABE UNBOUNDED/5X,26HOPTIMIZATION 	&
     2IS TERMINATED)
990   FORMAT (5X,17HCONSTRAINT NUMBER,I5)     
!990   FORMAT (5X,17HCONSTRAINT NUMBER,I5)+				! This + was creating problems
									! with the intel f90 compiler
									! removed it 2.2.03.
									! The 1 and 2 seem to act as 
									! continuation symols in col 6
...
C	ASH
1170  FORMAT (10X,62HTEN CONSECUTIVE ITERATIONS FAILED TO PRODUCE A FEAS	&
     1IBLE DESIGN)
1190  FORMAT (10X,43HABS(1-OBJ(I-1)/OBJ(I)) LESS THAN DELFUN FOR,I3,11H 	&
     1ITERATIONS)
1200  FORMAT (10X,43HABS(OBJ(I)-OBJ(I-1))   LESS THAN DABFUN FOR,I3,11H 	&
     1ITERATIONS)
1210  FORMAT (/5X,22HNUMBER OF ITERATIONS =,I20)
1220  FORMAT (/5X,28HCONSTRAINT PARAMETER, BETA =,E14.5)
1230  FORMAT (1H1,////12X,27(2H* )/12X,1H*,51X,1H*/12X,1H*,20X,11HC O N		&
...
1240  FORMAT (////5X,33HCONSTRAINED FUNCTION MINIMIZATION//5X,18HCONTROL	&
     1 PARAMETERS)
1250  FORMAT (/5X,60HIPRINT  NDV    ITMAX    NCON    NSIDE  ICNDIR   NSC	&
     1AL   NFDG/8I8//5X,12HLINOBJ  ITRM,5X,2HN1,6X,2HN2,6X,2HN3,6X,2HN4,	&
     26X,2HN5/8I8)
1260  FORMAT (/9X,4HFDCH,12X,5HFDCHM,11X,6HALPHAX,10X,6HABOBJ1/1X,4(2X,E	&
     114.5))
1270  FORMAT (/9X,2HCT,14X,5HCTMIN,11X,3HCTL,13X,6HCTLMIN/1X,4(2X,E14.5)
     1//9X,5HTHETA,11X,3HPHI,13X,6HDELFUN,10X,6HDABFUN/1X,4(2X,E14.5))
1280  FORMAT (/5X,40HLOWER BOUNDS ON DECISION VARIABLES (VLB))
1290  FORMAT (/5X,40HUPPER BOUNDS ON DECISION VARIABLES (VUB))
1300  FORMAT (////5X,35HUNCONSTRAINED FUNCTION MINIMIZATION//5X,18HCONTR	&
     1OL PARAMETERS)
1310  FORMAT (/5X,21HSCALING VECTOR (SCAL))
1320  FORMAT (////5X,22HBEGIN ITERATION NUMBER,I5)
1330  FORMAT (/5X,4HCT =,E14.5,5X,5HCTL =,E14.5,5X,5HPHI =,E14.5)
1340  FORMAT (/5X,25HNEW SCALING VECTOR (SCAL))
1350  FORMAT (/5X,15HGRADIENT OF OBJ)
1360  FORMAT (/5X,44HGRADIENTS OF ACTIVE AND VIOLATED CONSTRAINTS)
1370  FORMAT (5H     )
1380  FORMAT (/5X,37HPUSH-OFF FACTORS, (THETA(I), I=1,NAC))
1390  FORMAT (/5X,27HSEARCH DIRECTION (S-VECTOR))
1400  FORMAT (/5X,18HCALCULATED ALPHA =,E14.5)
1410  FORMAT (////5X,6HITER =,I5,5X,5HOBJ =,E14.5,5X,16HNO CHANGE IN OBJ	&
     1)
1420  FORMAT (/5X,5HOBJ =,E15.6,5X,16HNO CHANGE ON OBJ)
1430  FORMAT (/5X,5HOBJ =,E15.6)
1440  FORMAT (////5X,6HITER =,I5,5X,5HOBJ =,E14.5)
1450  FORMAT (//5X,28HINITIAL FUNCTION INFORMATION//5X,5HOBJ =,E15.6)
1460  FORMAT (/5X,29HDECISION VARIABLES (X-VECTOR))
1470  FORMAT (3X,7E13.4)
1480  FORMAT (/5X,28HCONSTRAINT VALUES (G-VECTOR))
1490  FORMAT (5X,15I5)
1500  FORMAT (/5X,59HTHE NUMBER OF ACTIVE AND VIOLATED CONSTRAINTS EXCEE	&
     ...
     3 TO MAIN PROGRAM.)
1510  FORMAT (1H1,////4X,30HFINAL OPTIMIZATION INFORMATION)
1520  FORMAT (/5X,32HOBJECTIVE FUNCTION WAS EVALUATED,8X,I10,2X,5HTIMES)
1530  FORMAT (/5X,35HCONSTRAINT FUNCTIONS WERE EVALUATED,I10,2X,5HTIMES)
1540  FORMAT (/5X,36HGRADIENT OF OBJECTIVE WAS CALCULATED,I9,2X,5HTIMES)
1550  FORMAT (/5X,40HGRADIENTS OF CONSTRAINTS WERE CALCULATED,I5,2X,5HTI	&
     1MES)
      END
     ...
     *,ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,IT
     *RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
      DIMENSION X(N1),DF(N1),G(N2),ISC(N2),IC(N3),A(N1,N3),G1(N2),VLB(N1	&
     1),VUB(N1),SCAL(N1),NCAL(2),C(N4)
...
70    S(I)=S1*S(I)
      ...
      RETURN
      END
      SUBROUTINE CNMN03 (X,S,SLOPE,XI,ALP,FFF,A1,A2,A3,A4,F1,F2,F3,F4,AP
     *P,N1,NCAL,KOUNT,JGOTO)
      IMPLICIT REAL*8 (A-H,O-Z)
      COMMON /CNMN1/ DELFUN,DABFUN,FDCH,FDCHM,CT,CTMIN,CTL,CTLMIN,ALPHAX
     *,ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,IT
     *RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
...
60    CONTINUE
     ...
     *,ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,IT
     *RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
      DIMENSION DF(N1),G(N2),ISC(N2),IC(N3),A(N1,N3),S(N1),C(N4),MS1(N5)
     1,B(N3,N3)
...
180   FORMAT (//5X,46H* * DIRECTION FINDING PROCESS DID NOT CONVERGE/5X,
     ...
     *ALP,A2,A3,A4,F1,F2,F3,CV1,CV2,CV3,CV4,ALPCA,ALPFES,ALPLN,ALPMIN,AL
     *PNC,ALPSAV,ALPSID,ALPTOT,ISC,N1,N2,NCAL,NVC,ICOUNT,IGOOD1,IGOOD2,I
     *GOOD3,IGOOD4,IBEST,III,NLNC,JGOTO)
      IMPLICIT REAL*8 (A-H,O-Z)
      COMMON /CNMN1/ DELFUN,DABFUN,FDCH,FDCHM,CT,CTMIN,CTL,CTLMIN,ALPHAX
     *,ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,IT
     *RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
